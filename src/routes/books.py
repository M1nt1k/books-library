...
@router.post('/', response_model=models.BookModel)
def add_doc(doc: models.BookModel):
...
@router.get('/{doc_id}', response_model=models.BookModel)
...
@router.get('/publisher/{doc_id}')
def get_doc(doc_id: str):
    if (doc :=  db.publishers.find_one({"_id": ObjectId(doc_id)})) is not None:
        del doc['_id']

        res = []
        docs_res = col.find({"publisher": doc})
        for doc in docs_res:
            doc['_id'] = str(doc['_id'])
            res.append(doc)
        return res
...
@router.get('/age/{doc_id}')
def get_doc(doc_id: str):
    if (doc :=  db.ages.find_one({"_id": ObjectId(doc_id)})) is not None:
        del doc['_id']

        res = []
        docs_res = col.find({"age": doc})
        for doc in docs_res:
            doc['_id'] = str(doc['_id'])
            res.append(doc)
        return res
